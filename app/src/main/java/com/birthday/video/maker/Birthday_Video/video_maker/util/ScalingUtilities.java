...
public class ScalingUtilities {
    ...
    public static Bitmap ConvetrSameSize(Bitmap originalImage, Bitmap bgBitmap, int mDisplayWidth, int mDisplayHeight, float x, float y) {
        ...
        Canvas blurCanvas;
        try {
            blurCanvas = new Canvas((FastBlur.fastblur(cs, 25, true)));
        } catch (Exception e) {
            e.printStackTrace();
            blurCanvas = new Canvas(cs);
        }

        blurCanvas.drawBitmap(newscaleBitmap(originalImage, mDisplayWidth, mDisplayHeight, x, y), 0.0f, 0.0f, null);
        return cs;
    }
    ...
    public static Bitmap checkBitmap(String path)
    {
        ...
        Bitmap vvvv= BitmapFactory.decodeFile(path, bounds);
        ...
        } catch (IOException e)
        {
            e.printStackTrace();
            exception=true;
            Bitmap bitmap= Bitmap.createBitmap(BirthdayWishMakerApplication.VIDEO_WIDTH, BirthdayWishMakerApplication.VIDEO_HEIGHT, Config.ARGB_4444);
            return bitmap;
        }
    }
